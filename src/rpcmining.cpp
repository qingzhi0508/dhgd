...
extern volatile bool fForgeblock;
...
Value setforgeblock(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            "setforgeblock <generate>\n"
...
Value getstakinginfo(const Array& params, bool fHelp)
{
    ...
    bool staking = fForgeblock && nWeight;
    ...
    obj.push_back(Pair("stakingfrozen", pwalletMain->GetStake()));
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "dhg is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "dhg is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "dhg is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "dhg is downloading blocks...");
