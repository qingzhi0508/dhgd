...
#if defined(NDEBUG)
# error "dhg cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "dhg Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // dhg: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// dhg: find block wanted by given orphan block
...
// dhg: find last block index up to pindex
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal dhg miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // dhg: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // dhg: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // dhg: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // dhg: track money supply and mint amount info
...
// dhg: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // dhg: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // dhg: compute stake entropy bit for stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // dhg: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// dhg: attempt to generate suitable proof-of-stake
