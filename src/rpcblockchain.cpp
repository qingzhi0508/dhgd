...
Value getlatestblocks(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getblockbynumber <number> [txinfo]\n"
    ...
    int blocks = params[0].get_int();
    if (blocks < 1 || blocks > 20)
        throw runtime_error("Block numbers out of range.");

    CBlock block;
    CBlockIndex* pblockindex = mapBlockIndex[hashBestChain];
    int nStartHeight = pblockindex->nHeight - blocks + 1;
    if(nStartHeight < 0)
        nStartHeight = 0;
    ...
    while (pblockindex->nHeight >= nStartHeight)
    {
        ...
        pblockindex = mapBlockIndex[hash];
        block.ReadFromDisk(pblockindex, true);
        ret.push_back(blockToJSON(block, pblockindex, false));
...
// dhg: get information of sync-checkpoint
